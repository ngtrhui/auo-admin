...
import { uploadBlogAuthorAvatarToR2 } from '@/lib/r2';
...
export async function POST(
  ...
  if (file.size > MAX_AVATAR_SIZE_BYTES) {
    return NextResponse.json(
      { message: 'Ảnh vượt quá dung lượng cho phép (5MB).' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const url = await uploadBlogAuthorAvatarToR2(normalizedBlogId, buffer, contentType);
